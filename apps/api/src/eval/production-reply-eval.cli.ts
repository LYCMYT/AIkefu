...
async function main(): Promise<void> {
  ...
  if (offline) for (const key of AI_ENVIRONMENT_KEYS) delete process.env[key];
  ...
  try {
    ...
    writeReports(source.version, runner, report);
    process.stdout.write(`${mode}: ${report.summary.passed}/${report.summary.total} passed; ${report.summary.failed} failed\n`);
    process.stdout.write(`Reports: artifacts/eval/reply-eval-${mode.toLowerCase()}-latest.{json,md}\n`);
...
function loadCases(): EvalFile {
  const source = JSON.parse(readFileSync(resolve(process.cwd(), 'seed/eval-cases.json'), 'utf8')) as EvalFile;
...
function writeReports(
  ...
  const prefix = `reply-eval-${report.mode.toLowerCase()}`;
