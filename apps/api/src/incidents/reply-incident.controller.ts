...
import { IncidentCorrectionDto, IncidentCreateDto, IncidentRegressionDto, IncidentRootCauseDto } from '../common/request-dtos';
...
@Controller() export class ReplyIncidentController {
  ...
  async create(@CurrentWorkspace() s: AuthenticatedWorkspace, @Param('replyId') replyId: string, @Body() b: IncidentCreateDto) {
  ...
  async root(@CurrentWorkspace() s: AuthenticatedWorkspace,@Param('id') id:string,@Body() b:IncidentRootCauseDto) { const incident = await this.incidents.rootCause(await this.incidents.scopeForIncident(s, id), id, b.rootCause); return { status: 'ACCEPTED' as const, operationId: incident.id }; }
  @Post('incidents/:id/correction') @HttpCode(HttpStatus.ACCEPTED)
  async correction(@CurrentWorkspace() s: AuthenticatedWorkspace,@Param('id') id:string,@Body() b:IncidentCorrectionDto) {
  ...
  async regression(@CurrentWorkspace() s: AuthenticatedWorkspace,@Param('id') id:string,@Body() b:IncidentRegressionDto) { const result = await this.incidents.regression(await this.incidents.scopeForIncident(s, id), id, b); return { status: 'ACCEPTED' as const, operationId: result.incident.id, evalCaseId: result.evalCaseId }; }
