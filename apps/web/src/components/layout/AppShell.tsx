...
export function AppShell({ activePath, activeShopId, children, isResetting, onNavigate, onReset, onShopChange, onTraceToggle, routeTitle, shops, socketLabel, socketReady, traceOpen, workspaceId }: AppShellProps) {
  ...
  return <main className={`app-shell route-${activePath.slice(1)}`}>
    <aside aria-label="主导航" className="sidebar">
      ...
      <div className="workspace-switcher" title="当前演示数据隔离范围"><span className="workspace-avatar">W</span><div><small>当前 Workspace</small><strong>{shortId(workspaceId)}</strong></div></div>
      <nav aria-label="Primary" className="navigation">{navigationItems.map((item) => {
        ...
        return <a aria-current={selected ? 'page' : undefined} className={`nav-item ${selected ? 'is-active' : ''}`} href={item.path} key={item.path} onClick={(event) => { event.preventDefault(); onNavigate(item.path); }}><Icon aria-hidden="true" className="nav-icon" size={19} /><span className="nav-label"><strong>{item.label}</strong></span></a>;
      })}</nav>
    ...
    <section className="content">
      <header className="topbar"><div className="topbar-title"><div className="breadcrumb"><span>AIkefu</span><i>/</i><span>{activeNav.label}</span></div><h1>{routeTitle}</h1></div><div className="topbar-actions">
        ...
        {activePath === '/workbench' && <button className={`trace-toggle ${traceOpen ? 'is-on' : ''}`} onClick={onTraceToggle} type="button"><Eye aria-hidden="true" size={16} />调试</button>}
        <Button aria-label="重置演示" className="shell-reset-button" disabled={isResetting} onClick={onReset} variant="secondary"><RefreshCw aria-hidden="true" className={isResetting ? 'is-spinning' : ''} size={16} />{isResetting ? '重置中…' : '重置演示'}</Button>
        <span aria-label={`服务状态：${socketLabel}`} className={`connection-pill ${socketReady ? 'is-ready' : ''}`}><i />{socketLabel}</span>
      </div></header>
      {children}
      <footer className="page-footer"><span>AIkefu 智能客服控制台</span><span>服务已连接 · {socketLabel}</span></footer>
    </section>
  </main>;
}
