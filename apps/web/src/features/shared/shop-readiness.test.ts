...
describe('projectedShopAiReadiness', () => {
  ...
  it('does not advertise READY before copied settings are confirmed', () => {
    expect(projectedShopAiReadiness({ ...shop, settingsConfirmed: false }, { id: 'job-1', status: 'SUCCEEDED' })).toBe('PREPARING');
  });
