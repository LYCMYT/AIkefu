...
test('portfolio release documentation is complete and does not advertise localhost as public', () => {
  ...
  assert.deepEqual(unitCounts, [708, 708, 708]);
  assert.match(releaseNotes, /64 \/ 64/);
  assert.match(resumeCopy, /64 \/ 64/);
  ...
  assert.match(releaseNotes, /174\.933 秒/);
  assert.match(releaseNotes, /\+45%/);
  assert.match(releaseNotes, /29 (?:条|个|项)[^\n]*(?:cue|字幕)/i);
  assert.match(releaseNotes, /硬字幕[^\n]*外部 SRT[^\n]*(?:无软字幕轨|不含软字幕轨)/i);
  assert.match(releaseNotes, /9,165,137 bytes/);
  assert.match(releaseNotes, /E8BFA8D0E41CBEEDC1F186C3497200BDDF46B682D7BBC0608764F70DF9CEB9DC/i);
  assert.match(releaseNotes, /不冒充真人配音/);
