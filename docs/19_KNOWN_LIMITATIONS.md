...
22. 本机 DeepSeek Chat 适配器已完成结构化探针、合成浏览器主链及隔离生产评测。固定集 Offline / DeepSeek 均为 36/36，独立 AUTO 集两种模式均为 10/10。Embedding 仍为本地 1536 维 fallback，外部图片分析只允许服务端显式 opt-in，Judge 尚未形成完整外部 Gate。
23. Production Eval CLI 已接入真实 AppModule、ReplyRuntime、PostgreSQL Knowledge/Evidence、动态工具、provider fault、审批上下文变化、恢复驱动以及 durable Draft/SendOutbox/Receipt/Message 投影。36/36 是当前固定合成用例结果，不代表开放域准确率或生产 SLA。
