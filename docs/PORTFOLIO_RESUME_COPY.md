...
- 建立可复现质量门禁：708 / 708 单元测试、64 / 64 集成测试；Playwright 覆盖 27 个唯一用例，真实环境为 23 passed / 4 skipped / 0 failed，离线模式为 6 passed / 21 skipped / 0 failed，并用真实 PostgreSQL/pgvector、Redis、MinIO 验证持久化链路；通过 [`v1.1.0-demo`](https://github.com/LYCMYT/AIkefu/releases/tag/v1.1.0-demo) 固定可复验版本与演示视频。
...
[`v1.1.0-demo` 三分钟演示](https://github.com/LYCMYT/AIkefu/releases/download/v1.1.0-demo/aikefu-3min-demo.mp4)为 174.933 秒、1920×1080、30 fps 的 H.264/AAC 成片，聚焦 SC01–SC04 四条核心业务链、E017 质量回归与脱敏 Trace；使用 `zh-CN-XiaoxiaoNeural` `+45%` 在线神经语音和同源硬字幕/SRT，并明确不是真人配音。完整 `/showcase` 目录另提供 SC01–SC06 六场景。项目没有公网全栈体验地址，视频不等同于真实平台接入或生产部署。
